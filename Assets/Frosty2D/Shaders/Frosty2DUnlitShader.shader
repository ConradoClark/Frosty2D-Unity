Shader "Frosty/Unlit"
{
	Properties
	{
		...
		[MaterialToggle] [HideInInspector] _FadeInStripsHorizontal("FadeIn - HorizontalStrips", float) = 0
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				if (_FadeInStripsHorizontal) {
					bool reverse = false;
					bool reverse2 = true;

					float uvx = (reverse ? _MainTex_ST.x : 0) + i.uv.x * (reverse ? -1 : 1);
					float uvy = (reverse2 ? _MainTex_ST.y : 0) + i.uv.y * (reverse2 ? -1 : 1);
					float splits = 1;
					float speed = 2;

					float checker = fmod(uvx, splits) > _Time[1] * splits * speed ? -1 : 1;
					checker = checker == -1 ? checker : fmod(uvy, splits) > _Time[1] * splits * speed ? -1 : 1;
